...
int main() {
 ...
 chain->Add("HToAATo2Mu2B.root");
    ...
    // --- Histograms ---
    TH1F* h_muon_pt = new TH1F("h_muon_pt", ";Muon p_{T} [GeV]", 50, 0, 200);
    TH1F* h_muon_eta = new TH1F("h_muon_eta", ";Muon #eta", 50, -3, 3);
    TH1F* h_jet_pt  = new TH1F("h_jet_pt",  ";Jet p_{T} [GeV]", 25, 10, 160);
    TH1F* h_jet_eta = new TH1F("h_jet_eta", ";Jet #eta", 50, -3, 3);
    TH1F* h_dimu_mass = new TH1F("h_dimu_mass", ";Di-muon Mass [GeV]", 80, 0, 200);
    TH1F* h_dijet_mass = new TH1F("h_dijet_mass", ";Di-jet Mass [GeV]", 80, 0, 200);
    TH1F* h_higgs_mass = new TH1F("h_higgs_mass", ";Higgs Candidate Mass [GeV]", 80, 0, 300);
    TH1F* h_muon_pt_tight = new TH1F("h_muon_pt_tight", ";Muon p_{T} [GeV]", 50, 0, 200);

    TH1F* h_jet_pt_tight = new TH1F("h_jet_pt_tight", ";Jet p_{T} [GeV]", 25, 10, 160);

    TH2F* h2_dimu_dijet = new TH2F("h2_dimu_dijet", "Dimuon Mass vs Dijet Mass;M_{μμ} [GeV];M_{bb} [GeV]", 80, 0, 200, 80, 0, 200);

    // --- Event loop ---
    while (reader.Next()) {
        if (*nMuon < 2 || *nJet < 2) continue; // at least 2 muon and 2 jet
...
// --- Save output ---
    ...
    h_muon_pt->Write(); h_muon_eta->Write();
    h_jet_pt->Write();  h_jet_eta->Write();
    h_dimu_mass->Write(); h_dijet_mass->Write(); h_higgs_mass->Write();
    h_muon_pt_tight->Write();  h_jet_pt_tight->Write();
    h_jet_pt_tight->Write();

    h2_dimu_dijet->Write();
    fout.Close();

    std::cout << "Histograms with Muons, Jets, and Higgs candidate written to eff_output.root" << std::endl;
}
